...
//void WriteOutputNetCDF::write_all_cells_output(out_data_struct *all_out_data, const dmy_struct* dmy, int dt, const ProgramState* state) {
//
//  if (netCDF == NULL) {
...
//        }
//      } catch (std::exception& e) {
//        fprintf(stderr, "Error writing variable: %s, at timeIndex: %d\n", state->output_mapping.at(all_out_data[dataFiles[file_idx]->varid[var_idx]].varname).name.c_str(), (int)timeIndex);
//        throw;
//      }
//    }
//  }
